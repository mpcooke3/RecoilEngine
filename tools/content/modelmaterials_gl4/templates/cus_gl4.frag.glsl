...
void main(void){
	...
	*/
	...
	//outColor.rgb = iblSpecular;
		...
		// MAC FIX: features (trees, wrecks, debris) get UNITUNIFORMS.userDefined[0].x
		// = 0 by default. The original gate `buildProgress > -0.5` admits that 0 and
		// runs the under-construction visual effects on every feature in the world,
		// which blends BAR's `pulseTeamColor * line * sintimefast * 2.0` into
		// outSpecularColor (line 1265). For features, teamCol on macOS reads as the
		// player's bright-blue team colour rather than the GAIA neutral value, so
		// trees, wrecks etc. render bright cyan-blue from above. Tightening the
		// gate to require buildProgress > 0.001 excludes the feature-default case
		// while preserving real construction visuals (which jump above 0 within
		// one tick of being placed).
